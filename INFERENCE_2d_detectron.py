...
for input_path in list_folder:
    ...
    for i in range(0, len(images), 1):
                      ...
                      for pred in outputs["instances"].pred_masks.to("cpu"):
                          ...
                          if len(cc) == 0:
                             continue;
                              ...
                              segmentation[obj_coords[:, 0], obj_coords[:, 1], obj_coords[:, 2]] = 1
        ...
        imsave(sav_dir + filename +'_OUTPUT_SEG.tif', np.uint8(segmentation))
        
        max_seg = plot_max(segmentation, ax=0, fig_num=2)
        imsave(sav_dir + filename + '_OUTPUT_SEG_max_proj.tif', np.uint8(max_seg))      
