...
#name = '_quads_PARANODES_3D'

#name = '_quads_PARANODES_3D_shortened_data_0_25_overlap'

name = '_quads_PARANODES_3D_shortened_SMALL'
            ...
            for x in range(1, width + quad_size, round(quad_size - quad_size * overlap_percent)):
                 ...
                 for y in range(1, height + quad_size, round(quad_size - quad_size * overlap_percent)):
                      ...
                      for z in range(1, depth_im + quad_depth, round(quad_depth - quad_depth * overlap_percent)):
                          ...
                          #cleaned_seg = clean_edges(quad_truth, quad_depth, w=quad_size, h=quad_size, extra_z=0, extra_xy=20)
                          ...
                          imsave(sav_dir_max_project + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_quad_INPUT_max_proj.tif', np.uint8(max_quad_intensity))
                          #imsave(sav_dir_max_project + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_quad_TRUTH_max_proj.tif', np.uint16(max_quad_truth))                          

                          imsave(sav_dir_max_project + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_quad_TRUTH_max_proj.tif', np.uint8(max_quad_truth))      



                          imsave(sav_dir + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_quad_INPUT.tif', np.uint8(quad_intensity))
                          #imsave(sav_dir + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_quad_TRUTH.tif', np.uint16(quad_truth))
...
sav_dir = input_path + '/' + foldername + name + '/normalize/'    
try:
    # Create target Directory
    os.mkdir(sav_dir)
    print("Directory " , sav_dir ,  " Created ") 
