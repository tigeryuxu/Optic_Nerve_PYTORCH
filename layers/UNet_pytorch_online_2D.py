...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  4 23:50:12 2021
...
class UNetConvBlock(nn.Module):
    def __init__(self, in_size, out_size, padding, batch_norm, batch_norm_switchable, kernel_size):
        ...
        block.append(nn.Conv2d(in_size, out_size, kernel_size=kernel_size, padding=int(padding)))   # can be changed to 5
        block.append(nn.ReLU())
        if batch_norm:
            block.append(nn.BatchNorm2d(out_size))
        elif batch_norm_switchable:
            block.append(SwitchNorm2d(out_size))

        block.append(nn.Conv2d(out_size, out_size, kernel_size=kernel_size, padding=int(padding)))
        block.append(nn.ReLU())
        if batch_norm:
            block.append(nn.BatchNorm2d(out_size))
        elif batch_norm_switchable:
...
class UNetUpBlock(nn.Module):
    def __init__(self, in_size, out_size, up_mode, padding, batch_norm, batch_norm_switchable, kernel_size):
        ...
        if up_mode == 'upconv':
            self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=2, stride=2, output_padding = 0)
